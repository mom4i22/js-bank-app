...
const startLogoutTimer = function () {
  ...
  //Call time first time
  beginFirst();

  //Call timer every second
  const timer = setInterval(beginFirst, 1000);
  return timer;
};
...
const formatDate = function (date, locale) {
  ...
  const daysBetween = daysPassed(new Date(), date);
...
const displayMovements = function (account, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const date = new Date(account.movementsDates[i]); //Loop over movement dates
...
//Fake always logged in to test easily
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    // const locale = navigator.language; //Gives us browser's language
...
btnLoan.addEventListener('click', function (e) {
  ...
  if (amount > 0 && currentAccount.movements.some(mov => mov >= amount * 0.1)) {
    setTimeout(function () {
      ...
      clearInterval(timer);
      timer = startLogoutTimer();
    }, 3000); //Loan is approved after 3 seconds
  }
